...
버퍼는 생성되었지만, 아직 실제로 할당된 메모리는 없습니다. 버퍼에 메모리를 할당하기 위한 첫 번째 단계는 이름 그대로인 link:{vkGetBufferMemoryRequirements}[``vkGetBufferMemoryRequirements``] 함수를 사용하여 버퍼의 메모리 요구사항을 조회하는 것입니다.
...
메모리 할당에 성공했다면, 이제 {vkBindBufferMemory}[``vkBindBufferMemory``] 함수를 사용하여 이 메모리를 버퍼와 연결(바인딩)할 수 있습니다.
